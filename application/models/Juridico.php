...
class Application_Model_Juridico extends Application_Model_Model
{
    protected $name;
    ...
    {
        parent::__construct();
        $this->name = $this->prefix . '_juridico';
    }
    ...
    public function selectAll($filterState = 1, $like = NULL)
    {
        try {
            ...
            if(!is_null($like) && !empty($like)){
                $columns = array('u.usuario','u.data','u.tipo','u.descricao','u.processo');
                ...
                $select->orWhere($columns[4] . ' LIKE ?', '%'. $like .'%' );
            }
